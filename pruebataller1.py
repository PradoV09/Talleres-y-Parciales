...
class Personaje:
    ...
    def cambiar_tamaño(self, hongo):
        if hongo:
            ...
            print(f"{self.nombre_personaje} ha recogido un hongo, pero no esta grande.")
 
    def recoger_flor(self):
        if self.tamaño == "grande":
            self.poder_disparo = True
            self.arma = True
        if self.poder_disparo:
            print(f"{self.nombre_personaje} ha recogido una flor y ahora tiene el poder de disparo y un arma.")
        else:
            print(f"{self.nombre_personaje} ha recogido una flor, pero todavía no tiene el poder de disparo.")
    ...
    def incidente_cencillo(self):
        ...
        if self.tamaño == "pequeño" or incidente_sencillo:
            self.vidas -= 1
            print(f"{self.nombre_personaje} ha enfrentado un inicedente y ha perdido una vida.")
            if self.vidas <= 0:
                print(f"{self.nombre_personaje} ha perdido todas las vidas. Reiniciando personaje.")
                self.reiniciar_personaje()
        else:
    ...
    def incidente_grave(self):
        ...
        if self.tamaño == "grande" or grave:
            ...
            self.tamaño(False)
            print(f"{self.nombre_personaje} ha muerto. Reiniciando personaje.")
            self.reiniciar_personaje()
    ...
    def cambiar_nivel(self):
        if self.nivel < 4:
            self.nivel += 1
            print(f"{self.nombre_personaje} ha avanzado a nivel {self.nivel}")
        else:
            self.nivel = 1
            if self.mundo < 8:
                self.mundo += 1
                print(f"{self.nombre_personaje} ha avanzado a el mundo {self.mundo}")
            else:
                self.mundo = 1
...
if personaje_elegido is not None:
  print(f"Has elegido a {personaje_elegido.nombre_personaje}. ¡Que empiece la aventura!")
...
while True:
    ...
    if opcion == "0":
        ...
        cantidad_monedas = random.randint(1, 100)  
